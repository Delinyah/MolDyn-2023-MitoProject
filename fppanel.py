...
def create_panel(subfolder):
    ...
    draw.text((10, 5), title_text, font=font, fill=(0, 0, 0))
    ...
    title_draw.text((10, 5), title_text, font=font, fill=(0, 0, 0))
    panel.paste(title_image, (0, 0))

    # Paste images onto the panel
    x_offset = title_width
    space_between_images = 250  # number of pixels to move images closer
    for img in images:
        panel.paste(img, (x_offset, 0))
        x_offset += img.size[0] - space_between_images

    # Save the panel image in the subfolder
    panel.save(os.path.join(subfolder, f"{os.path.basename(subfolder)}-panel.png"))
